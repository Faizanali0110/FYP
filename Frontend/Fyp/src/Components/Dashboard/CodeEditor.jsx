...
export function CodeEditor({ file, onCodeChange }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      if (!file) {
        setCode("");
        // propagate clear to parent so dependent buttons can enable/disable correctly
      ...
      if (typeof content === "string") {
        ...
        if (!cancelled && onCodeChange) onCodeChange(content);
        return;
      }
      if (content && typeof content.text === "function") {
        try {
          ...
          if (!cancelled && onCodeChange) onCodeChange(text);
          return;
        } catch (e) {
      ...
      if (!cancelled && onCodeChange) onCodeChange("// Start coding...");
    };
    load();
    return () => {
      cancelled = true;
  ...
  const handleEditorChange = (value) => {
    setCode(value ?? "");
    // Notify parent component about code changes
    if (onCodeChange) {
      onCodeChange(value ?? "");
    }
  };
